<#
  MCHC - repair:
   1) Re-download the Fabric mods (server + client) robustly via the Modrinth API.
   2) Rewrite the config files WITHOUT BOM (otherwise the properties parser may choke).
  Safe to run again.
#>
...
# Download the latest matching Fabric version of a Modrinth mod (client-side filtering = reliable).
function Get-Mod($slug, $dest){
    ...
    try {
        ...
        Write-Host ("  [API error] {0}: {1}" -f $slug, $_.Exception.Message) -ForegroundColor Red
        return
    }
    ...
    if(-not $pick){ Write-Host "  [no version] $slug" -ForegroundColor Red; return }
    ...
    try {
        ...
        Write-Host ("  [download error] {0}: {1}" -f $slug, $_.Exception.Message) -ForegroundColor Red
    }
    Start-Sleep -Milliseconds 300   # be nice to the Modrinth API
}

Write-Host "== Server mods (alpha + beta) ==" -ForegroundColor Cyan
# No proxy anymore: we use the vanilla transfer packet, so no fabricproxy-lite.
...
Write-Host "== Rewriting configs without BOM ==" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Done. Contents of alpha\mods:" -ForegroundColor White
